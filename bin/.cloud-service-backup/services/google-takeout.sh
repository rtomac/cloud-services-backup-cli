...
folder. Backups from archives into the backup folder will
only ever add new files, never delete files, as archives
often contain subsets of files to back up.
...
function svc_google_takeout_init {
    ...
    user_backupd_files=${user_backupd}/takeout
    rclone_remote=google_drive+${user_slug}

    mkdir -p "${user_backupd}"
    mkdir -p "${user_backupd_archives}"
    mkdir -p "${user_backupd_files}"
...
function svc_google_takeout_backup {
    ...
    echo "Running rclone ${subcommand} to download/sync archives..."
    ...
    _svc_google_takeout_cleanup_extracts
...
function _svc_google_takeout_extract_archives {
    for file in "$user_backupd_archives"/*.tgz "$user_backupd_archives"/*.zip; do
        ...
        if [ ! -d "$archive_dir" ]; then
            ...
            echo "Skipping $filename, extract folder already exists"
        fi
    done
}

function _svc_google_takeout_cleanup_extracts {
    for dir in "$user_backupd_archives"/*/; do
        [ -d "$dir" ] || continue
        dirname="$(basename "$dir")"
        if [ ! -f "$user_backupd_archives/$dirname".* ]; then
            echo "Deleting folder $dirname, no corresponding archive found"
            rm -rf "$dir"
        fi
    done
}
...
function _svc_google_takeout_sync_archives {
    ...
    for archive_dir in "$user_backupd_archives"/*/; do
        [ -d "$archive_dir" ] || continue
        archive_dirname="$(basename "$archive_dir")"

        for dir in "${archive_dir}"Takeout/*/; do
            [ -d "$dir" ] || continue
            dirname="$(basename "$dir")"
            echo "Syncronizing files from $archive_dirname > $dirname dir..."
            rsync $rsync_flags -v "$dir" "$user_backupd_files/$dirname/"
        done
    done
}
