...
function git_ensure_access_token {
    repo_host=${1:?repo_host arg required}
    git_username=${2:?git_username arg required}
...
function git_mirror_repos {
    ...
    do
        ...
        if [ ! -d "${repo_dir}" ]; then
            ...
            git_x config --global credential.helper "store --file ${credentials_file}"
            git_x clone --mirror --config credential.helper="store --file ${credentials_file}" "${clone_url}" "${repo_dir}"
            git_x config --global --unset credential.helper
        else
            echo "Updated mirror of ${repo_name}"
            git_x --git-dir "${repo_dir}" remote update
        fi
    done
}
