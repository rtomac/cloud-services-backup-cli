...
from cloud_services_backup_cli.services.google_contacts import GoogleContacts
...
def service(tmp_path):
    ...
    with patch.dict(os.environ, {
        ...
        yield GoogleContacts("rtomac@gmail.com")


class TestGoogleContactsService:

    def test_user_confd_is_under_vdirsyncer_google_contacts(self, service):
        assert "vdirsyncer" in str(service.user_confd)
        assert "google_contacts" in str(service.user_confd)

    def test_user_backupd_is_under_google_contacts(self, service):
        assert "google_contacts" in str(service.user_backupd)
